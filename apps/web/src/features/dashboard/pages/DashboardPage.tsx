...
import { getProductSummary, type Product } from '../../products'
...
export function DashboardPage() {
  ...
  if (!query.data) {
    return (
      <main className="page-content">
        <div className="state-panel dashboard-state">Loading dashboard…</div>
      </main>
    )
  }

  const data = query.data
  const activeRate = data.total_products
    ? Math.round((data.active_products / data.total_products) * 100)
    : 0
    ...
    <main className="page-content">
      ...
      <section className="metric-grid" aria-label="Catalog summary">
        <article className="metric-card">
          ...
          <div>
            <p>Total products</p>
            <strong>{data.total_products.toLocaleString()}</strong>
            <small>{activeRate}% currently active</small>
          </div>
        </article>
        <article className="metric-card">
          ...
          <div>
            <p>Inventory value</p>
            <strong>{currency.format(Number(data.inventory_value))}</strong>
            <small>Across all stock on hand</small>
          </div>
        </article>
        <article className="metric-card">
          <span className="metric-icon purple" aria-hidden="true">
            #
          </span>
          <div>
            <p>Units in stock</p>
            <strong>{compactNumber.format(data.total_stock)}</strong>
        ...
        <article className="metric-card warning-card">
          ...
          <div>
            <p>Low stock</p>
            <strong>{data.low_stock_count.toLocaleString()}</strong>
            <small>Products with 25 units or less</small>
          </div>
        </article>
      </section>
        ...
        <article className="dashboard-card category-card">
          <div className="card-heading">
            <div>
              ...
              <p>Current units across your catalog</p>
            </div>
            ...
            {data.categories.slice(0, 6).map((category) => (
              <div className="category-bar" key={category.name}>
                ...
                <div className="bar-track">
                  <span
                    style={{ width: `${Math.max((category.stock / maxCategoryStock) * 100, 3)}%` }}
                  />
                </div>
      ...
      <section className="dashboard-card recent-card">
        <div className="card-heading">
          <div>
            ...
            <p>The latest changes across your products</p>
          </div>
          <Link to="/products">Open catalog</Link>
        </div>
        <div className="activity-list recent-list">
          {data.recently_updated.map((product) => (
